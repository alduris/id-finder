            ...
            if (threadmaster != null && waitingForResults)
            {
                if (!threadmaster.Running)
                {
                    ...
                    if (threadmaster.AbortReason == null)
                    {
                        ...
                        var button_save = new OpSimpleButton(new(button_copy.PosX + button_copy.size.x + 10f, button_copy.PosY), new(100f, 24f), "SAVE SEARCH") { description = "Save results to history" };
